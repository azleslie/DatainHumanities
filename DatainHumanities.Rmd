...
Welcome to this introductory workshop on data in the humanities! This workshop is designed as an introduction to identifying potential data in the wild of humanistic sources. We'll also practice what is often the most discouraging part of the whole process: translating these materials into usable formats. In the process, this workshop will provide a soft introduction to programming and the idea of data. We'll also survey some examples of scholarship that does this well and discuss some of the costs and benefits of translating into data. One of the goals is a reminder that humanists have always used data in scholarship - indeed, well before the advent of the database search made digital humanists of us all. 
...
That said, it's still possible to separate out one variable into two, but irregularities in your data can cause errors more readily. Here is a line of code that separates out first names, but if I had included periods after first initials ("J." instead of "J") it would require a bit more tweaking.
```{r}
gsub("(^\\w+) \\w+$", "\\1", names)
```

When inheriting spreadsheets from someone or somewhere else, these are two of the most common operations necessary for cleaning before analysis can begin. But there are a lot of even more powerful functions that can be carried out in an instant in a programming language like R that are much trickier otherwise. For example, you may want to transpose a spreadsheet, flipping the rows and columns, or you may have values in one column that should be separated out and spread across two columns. A simple `t` or `spread` function, respectively, would carry out these tasks in an instant.
...
There might be further implicit or noncharacter information contained in archival materials. This might include marginalia in printed documents or physical characteristics of materials. In this example, some names are crossed out, marking the termination of their subscription. Subscription books, like many other manuscript records, were living documents. Today sites like github.com keep track of version control; in the nineteenth century, editors added names and crossed out others until their ledgers became illegible, at which point they began new ones. It's impossible to tell when a name was crossed out here, but knowing the logic of the record, we can acknowledge that any unredacted names were still subscribers at the moment of transition into a new ledger. This might help us compare data and give us a glimpse into the temporality internal to the record.
```{r}
...
The greatest influence on my own interest in DH was William Cronon's *Nature's Metropolis*, which well predates the naming or debating of a "digital humanities" but nonetheless does superb .^[Cronon, William. *Nature's Metropolis*. New York: W. W. Norton & Co., 1991.] (Right, below.)
...
On the Gutenberg page for any given book, there will be several format options: the most amiable one is the most basic one, the "Plain Text UTF-8" format. Copying and pasting this into a plain notepad .txt document (not a Word .docx document) is all that's necessary to begin working with this text as data. Like a .csv file, this is the simplest and smallest format for plain text data, which makes it the most reliable to work with. And again, any text file in, say, Microsoft Word's .docx format can be turned into a .txt format in the "Save as" menu.

Even more easily, you might scrape this text directly off the web and into an object in your computer's memory. Here is the economist and sociologist Thorstein Veblen's *The Theory of the Leisure Class* (1899). We can read it into an object in memory with `readLines`.
```{r}
book <- readLines("http://www.gutenberg.org/cache/epub/833/pg833.txt")
```

Type "book" into your console and click enter. If you scroll up to the top, you'll see a bunch of stuff that isn't part of the book proper: website metadata, text metadata, an introduction. We'll want to trim that stuff off.
If you went to the webpage we got it from, you could check to see that the book begins with a line that reads "FIRST PART" and ends with one that reads "Well, we must trust that look of hers." If we search the book object for elements containing those exact strings of characters, we'll find the exact position for the start and the end of our desired text. The `grep` function does this.
```{r}
...
We could just as easily split our text up into sentences. The only thing to change would be the regular expression, which now includes all the characters that signal the end of a sentence as triggers for splitting the character string. A period needs to be designated as `\\.` because it can mean other things in regular expressions (namely, any character whatsoever). Before doing that, though, it would be necessary to substitute out common periods that don't end sentences. The `gsub` function makes substitutions: the first input is the searched text, the second input is the desired replacement, and the third input is the object. This is essentially just like ctrl+f+replace, but the usefulness of doing this computationally really kicks in when you need to search for a regular expression rather than an exact character string.
```{r}
...
But Project Gutenberg, wonderful resource that it is, is unfortunately a relatively small collection of texts. It's a great place to look first, since it has the cleanest text anywhere, but it may well not include what you're looking for. Where else can you get texts from? There are several options. The best organized of these is [Hathitrust](https://www.hathitrust.org/).

![Hathitrust screenshot](images/hathitrust_options.png){ width=70% }

Hathitrust offers a couple different means of accessing texts. The default is an online PDF view. Hathitrust has a PDF download option that allows you to either "Download whole book (PDF)" or "Download pages (PDF)" by click the upper-right corner of each page that you want to select.

The quickest way to get the plain text of a Hathitrust item is by clicking "text-only view of this item." There isn't a direct way to download just this as one .txt, but you could just copy and paste it. Hathitrust does have a [browser API](https://babel.hathitrust.org/cgi/htdc) that allows downloads of the full text, but each page will be a separate .txt file in a folder that also contains an image and an .xml file for each page - which ends up making it pretty large. Now, if you have plenty of hard drive space, this could be a useful format.

The trouble with this Hathitrust view is that it turns each page into a single line of text. This would be a problem if you were working with something like poetry, where line breaks matter. Blank space is data too. But this view presents other issues as well, such as the way the running title gets lumped in with the body text. Hathitrust does offer several other options to experienced computational researchers for obtaining large numbers of texts, but they're much more advanced.

It is also possible to turn a downloaded .pdf file - from Hathitrust or elsewhere - into a .txt file. Adobe Acrobat software available on campus computers can carry this out, as can some other free programs, such as [PDF-XChange Editor](https://www.tracker-software.com/product/pdf-xchange-editor) for Windows, which I've found useful, or [pdftotext](https://pypi.org/project/pdftotext/) for Mac, which Francesca recommended.

For more focused work that extends beyond the offerings of Gutenberg, I recommend the [Internet Archive](https://archive.org/), another non-profit and largely open-access database of, among other things, 20 million texts (many of which are the same files available on Hathitrust).

Conveniently, Internet Archive has pre-generated pages with only text for each document. To access this, click the "show all" button in the "Download Options" box on the page for any item. On the subsequent page select the option that ends with .txt.

![Internet Archive screenshot](images/internet_archive_options.png){ width=60% }

Here is Priscilla Jane Thompson's *Ethiope Lays* (1900).
```{r}
...
Since `grep` and `agrep` are inclusive, they'll also grab longer lines that include the desired (or approximate) character string. To filter this out just requires a couple `nchar` functions.
```{r}
running_titles <- agrep("Ethiope Lays. ", volume, max.distance=5, ignore.case=TRUE)
...
# What Are You Going to Do with That, Redux
