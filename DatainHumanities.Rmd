...
Welcome to this introductory workshop on data in the humanities! This workshop is designed as an introduction to the first and often highest hurdle in the whole process: translating materials into usable data formats. In the process, this workshop will provide a soft introduction to programming, survey some examples of scholarship that works with data effectively, and discuss some of the costs and benefits of translating into data. One of the goals is a reminder that humanists have always used data in scholarship, even before the advent of the database search made digital humanists of us all. 
...
On the Gutenberg page for any given book, there will be several format options: the most amiable one is the most bare-bones, the "Plain Text UTF-8" format. Copying and pasting this into a plain notepad .txt document (not a Word .docx document) is all that's necessary to begin working with this text as data. Like a .csv file, this is the simplest and smallest format for plain text data, which makes it the most reliable to work with. And again, any text file in, say, Microsoft Word's .docx format can be turned into a .txt format in the "Save as" menu.

Even more easily, you might scrape this text directly off the web and into an object in your computer's memory with `readLines`. Here is Thorstein Veblen's *The Theory of the Leisure Class* (1899).
```{r}
...
We could just as easily split our text up into sentences. The only thing to change would be the regular expression, which now includes all the characters that signal the end of a sentence as triggers for splitting the character string. A period needs to be designated as `\\.` because it is a regular expression metacharacter (namely, any character whatsoever). Before doing so, it would be necessary to substitute out common periods that don't end sentences. The `gsub` function makes substitutions: the first input is the searched text, the second input is the desired replacement, and the third input is the object.
```{r}
...
To download the materials for this workshop, visit [https://github.com/azleslie/DatainHumanities](https://github.com/azleslie/DatainHumanities).
